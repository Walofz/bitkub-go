...
func fetchCurrentBalance() map[string]float64 {
	...
	if err != nil {
		...
		return map[string]float64{"THB": 0.0, "ETH": 0.0}
	}
	return balances
}
...
func CalculatePortfolio() PortfolioSummary {
	...
	ethPrice := fetchCurrentPrice("ETH")
	LatestEthPrice = ethPrice

	ethValue := balance["ETH"] * ethPrice
	totalValue := balance["THB"] + ethValue
	...
	for _, assetData := range portfolio {
		...
		if deviation > threshold {
			...
			if operation == "buy" {
				...
				if coinAmount < minEthAmount {
					fmt.Printf("⏸️ SKIP: SELL ปริมาณ %.8f ETH น้อยกว่าขั้นต่ำ %.4f ETH\n", coinAmount, minEthAmount)
					continue
				}
