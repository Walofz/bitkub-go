...
const API_HOST = "https://api.bitkub.com/api"
...
func sendPrivateRequest(endpoint string, method string, payload map[string]interface{}) ([]byte, error) {
	if APIKey == "YOUR_BITKUB_API_KEY" || APISecret == "YOUR_BITKUB_API_SECRET" {
	...
	req, _ := http.NewRequest(method, API_HOST+"/"+endpoint, bytes.NewBuffer(payloadBytes))
...
func FetchTickerPrice(sym string) (float64, error) {
	resp, err := http.Get("https://api.bitkub.com/api/market/ticker?sym=" + sym)
	if err != nil {
		return 0, err
	}
	defer resp.Body.Close()
